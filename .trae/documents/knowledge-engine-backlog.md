...
- **角色调整**:
    ...
    - **Auto-Coder (执行者)**: 同时查阅 Rules (长期) 和 Inbox Index (短期)。
...
- **方案**: 
    ...
    - 在 `auto-coder` Prompt 中内置 “关键词 -> 规则文件” 映射表。
- **来源**: 2026-03-05 访谈反馈
...
- **方案**: 
    ...
    - 示例: 在 `image-compression.ts.md` 中提供 `validateParams` 函数供 Auto-Coder 直接引用。
- **来源**: 2026-03-05 访谈反馈
...
**核心观点**:
1.  **Auto-Coder**: 成功强制查阅文档避免了配置错误，但 `<thinking>` 输出过于僵硬，规则定位成本高。
