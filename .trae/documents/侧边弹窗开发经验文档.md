...
### 6.6 Vue Mixin 方法被 app.js 覆盖问题案例

#### 遇到的问题

1. **弹窗内容不渲染**：素材编辑弹窗打开后，Konva 画布不显示任何内容（原始素材图、上传图片、文案）
2. **控制台无错误**：没有任何报错信息，但 Konva 调试日志也没有输出
3. **DOM 无 Konva 元素**：元素面板中看不到 Konva 创建的 div

#### 根本原因

**app.js 中重复定义了与 mixin 同名的方法**：Vue 组件中直接定义的方法会覆盖 mixin 中的同名方法。

```javascript
// app.js 中的定义（覆盖了 mixin）
openMaterialEditor: function (item) {
    window.MeeWoo.Core.MaterialOperations.openMaterialEditor(this, item);
    // 缺少 Konva 初始化逻辑！
}

// material-editor.js mixin 中的定义（被覆盖，未被执行）
openMaterialEditor: function (item) {
    window.MeeWoo.Core.MaterialOperations.openMaterialEditor(this, item);
    // 包含 Konva 初始化逻辑
...
**问题链路**：
```
app.js 定义了 openMaterialEditor 方法
       ↓
覆盖了 material-editor.js mixin 中的同名方法
       ↓  
mixin 中的 Konva 初始化逻辑被跳过
       ↓
initKonvaStage() 从未被调用
       ↓
Konva 画布未创建，内容不渲染
```
...
**删除 app.js 中的冗余方法定义**，让 mixin 的方法生效：
...
1. **避免重复定义 mixin 方法**：app.js 中不应定义与 mixin 同名的方法，除非有意覆盖
2. **添加新弹窗功能后检查**：开发新弹窗功能后，检查是否意外覆盖了 mixin 方法
3. **调试日志缺失是线索**：如果预期的调试日志没有输出，说明代码根本没有被执行，检查是否被覆盖
4. **Vue mixin 合并策略**：组件自身的方法优先级高于 mixin

#### 诊断方法

1. 检查控制台是否有预期的调试日志输出
2. 在 app.js 中搜索 mixin 方法名，检查是否有重复定义
3. 检查 `mixins: [...]` 配置是否正确引入

### 6.7 Vite开发服务器Worker加载问题案例
...
### 6.8 GitHub Pages COEP credentialless 模式问题案例（重要）

#### 遇到的问题

1. **GIF Worker被COEP屏蔽**：`gif.worker.js`状态显示"已屏蔽：受 COEP 限制的嵌入资源需要 COEP 标头"
2. **FFmpeg wasm加载失败**：从unpkg.com加载wasm时报`ERR_HTTP2_PROTOCOL_ERROR`
3. **修复冲突**：修复一个问题会导致另一个问题出现

#### 错误尝试

1. **错误方案1：为同源资源添加CORP头** → GIF Worker正常，但FFmpeg wasm失败
2. **错误方案2：跳过CDN请求** → FFmpeg正常，但GIF Worker被屏蔽
3. **错误方案3：对CDN请求显式fetch透传** → 仍然导致GIF Worker被屏蔽

#### 根本原因

**混淆了`credentialless`和`require-corp`两种COEP模式的处理方式**：

| COEP 模式 | 是否需要CORP头 | 适用场景 |
|-----------|---------------|----------|
| `require-corp` | 是，所有资源都需要CORP头 | 开发环境（Vite） |
| `credentialless` | 否，不需要CORP头 | 生产环境（GitHub Pages） |

**问题链路**：
```
在 credentialless 模式下错误地添加 CORP 头
       ↓
浏览器误认为需要严格检查资源策略
       ↓  
跨域CDN资源或同源Worker被错误屏蔽
       ↓
GIF导出或FFmpeg转换功能失效
```
...
**参考官方 gzuidhof/coi-serviceworker 实现**，`credentialless`模式的正确做法：

```javascript
// coi-serviceworker.js 正确实现
self.addEventListener('fetch', (event) => {
  ...
  event.respondWith(
    fetch(request).then((response) => {
      if (response.status === 0) return response; // opaque 响应直接返回
      
      const newHeaders = new Headers(response.headers);
      newHeaders.set('Cross-Origin-Opener-Policy', 'same-origin');
      newHeaders.set('Cross-Origin-Embedder-Policy', 'credentialless');
      // 注意：不设置 Cross-Origin-Resource-Policy！
...
**核心原则**：
- `credentialless`模式**不需要**为任何资源设置CORP头
- 只需统一设置COOP和COEP头
- 对`no-cors`请求省略凭据（`credentials: 'omit'`）
- opaque响应（`status === 0`）直接返回，不修改

#### 经验教训

1. **理解COEP模式差异**：`credentialless`和`require-corp`处理方式完全不同，不可混用
2. **开发与生产环境区分**：
   - Vite开发服务器：使用`require-corp` + CORP头
   - GitHub Pages生产：使用`credentialless` + 无CORP头
3. **参考官方实现**：遇到复杂的跨域策略问题，优先查找官方或成熟的开源实现
4. **Service Worker版本管理**：修改`coi-serviceworker.js`后，必须更新`index.html`中的版本号触发重注册

#### 诊断方法

1. 检查Network面板Worker脚本状态是否显示"受COEP限制"
2. 检查CDN资源是否报`ERR_HTTP2_PROTOCOL_ERROR`
3. 检查`coi-serviceworker.js`是否错误地设置了CORP头
4. 确认使用的COEP模式（`credentialless` vs `require-corp`）

#### 涉及文件

- `src/coi-serviceworker.js` - Service Worker实现
- `src/index.html` - Service Worker版本号
- `vite.config.js` - 开发环境中间件配置
...
- [ ] **检查 app.js 是否重复定义了 mixin 方法**
- [ ] 重试机制已添加
- [ ] 暗黑模式支持已添加
- [ ] 过渡动画已实现
- [ ] **OffscreenCanvas 使用 convertToBlob()**（不是 toBlob）
- [ ] **生产环境COEP使用credentialless模式**（不要设置CORP头）
...
13. **Vue mixin 方法覆盖**：app.js 中不应定义与 mixin 同名的方法，否则会覆盖 mixin 的逻辑。开发新弹窗功能后应检查是否意外覆盖
14. **COEP模式区分**：`credentialless`模式（GitHub Pages）**不需要**CORP头，`require-corp`模式（Vite开发）**需要**CORP头。两种模式处理方式完全不同，不可混用
