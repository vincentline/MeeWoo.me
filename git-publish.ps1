...
try {
    ...
    Write-Host "Error closing Node processes: $($_.Exception.Message)"
}
    ...
    if (-not $gitStatus) {
        ...
        if (Test-Path $gitPushScriptPath) {
            ...
            if ($LASTEXITCODE -eq 0) {
                ...
                Write-Host "Failed to commit changes, script will continue with publish operation"
            }
        } else {
            Write-Host "git-push.ps1 script not found, skipping commit step"
        }
    }
} catch {
    Write-Host "Error checking Git status: $($_.Exception.Message)"
    Write-Host "Script will continue with publish operation"
}
...
try {
    ...
    if ($LASTEXITCODE -ne 0) {
        Write-Host "Failed to separate docs directory: $splitHash"
        exit 1
    }
    ...
    if (-not $splitHash) {
        Write-Host "Failed to get separated commit hash"
        exit 1
    }
    ...
    Write-Host "Error publishing to gh-pages branch: $($_.Exception.Message)"
    exit 1
}
