...
(function (global) {
  ...
  global.MeeWoo.Service.PanelDrag = {
     ...
     */
    setupPanel: function (panelElement) {
  ...
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', () => {
      // 延迟初始化，确保Vue组件已经渲染完成
      setTimeout(() => {
        if (global.MeeWoo && global.MeeWoo.Service && global.MeeWoo.Service.PanelDrag) {
  ...
  const observer = new MutationObserver((mutations) => {
    try {
      ...
      console.error('[PanelDrag] MutationObserver error:', error);
    }
  });
  ...
  function startObserving() {
    if (document && document.body) {
      try {
        ...
        console.error('[PanelDrag] Observer error:', error);
      }
    } else {
