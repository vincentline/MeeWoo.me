...
| `docs/assets/js/core/material-interactions.js` | 素材交互逻辑，包含素材的交互操作 | 素材交互、交互操作 |
...
| `docs/assets/js/core/konva-command.js` | Konva 命令系统，用于画布操作的撤销/重做 | Konva、命令系统、撤销/重做 |
| `docs/assets/js/core/konva-element.js` | Konva 元素管理，用于创建和管理画布元素 | Konva、元素管理 |
| `docs/assets/js/core/konva-export.js` | Konva 导出功能，用于将画布内容导出为图片 | Konva、导出功能 |
| `docs/assets/js/core/konva-selection.js` | Konva 选择功能，用于选择和操作画布元素 | Konva、选择功能 |
| `docs/assets/js/core/konva-stage.js` | Konva 舞台管理，用于创建和管理画布舞台 | Konva、舞台管理 |
| `docs/assets/js/core/konva-transformer.js` | Konva 变形器，用于变换画布元素 | Konva、变形器、变换操作 |

### 3. 组件模块 (docs/assets/js/components/)

| 文件路径 | 功能描述 | 关键词 |
|---------|---------|--------|
| `docs/assets/js/components/index.js` | 组件入口文件，导出所有组件 | 组件入口、导出管理 |
...
| 文件路径 | 功能描述 | 关键词 |
|---------|---------|--------|
| `docs/assets/js/service/config-manager.js` | 配置管理器，管理应用配置 | 配置管理、配置加载、应用配置 |
| `docs/assets/js/service/dual-channel/` | 双通道 MP4 相关服务 | 双通道、MP4 服务、视频处理 |
| `docs/assets/js/service/dual-channel/dual-channel-composer.js` | 双通道合成器，用于合成双通道视频 | 双通道、视频合成 |
| `docs/assets/js/service/dual-channel/dual-channel-worker.js` | 双通道工作线程，用于后台处理双通道视频 | 双通道、工作线程 |
| `docs/assets/js/service/ffmpeg/` | FFmpeg 相关服务，用于视频转换 | FFmpeg、视频转换、服务模块 |
| `docs/assets/js/service/ffmpeg/ffmpeg-service.js` | FFmpeg 服务，提供视频转换功能 | FFmpeg、视频转换 |
| `docs/assets/js/service/gif/` | GIF 导出相关服务 | GIF 导出、服务模块、动画处理 |
| `docs/assets/js/service/gif/gif-exporter.js` | GIF 导出器，用于导出 GIF 动画 | GIF 导出 |
| `docs/assets/js/service/gif/gif.worker.js` | GIF 工作线程，用于后台处理 GIF 导出 | GIF、工作线程 |
...
| `docs/assets/js/service/oxipng/` | OXIPNG 相关服务，用于 PNG 优化 | OXIPNG、PNG 优化 |
...
| `docs/assets/js/service/svga/svga-builder.js` | SVGA 构建器，用于构建 SVGA 文件 | SVGA 构建、格式处理 |
...
| `docs/assets/js/utils/auth-utils.js` | 登录状态管理工具，处理登录状态和token管理 | 登录状态、token管理、认证工具 |
| `docs/assets/js/utils/image-api.js` | 图片处理API调用工具，与www.imghlp.com的API交互 | 图片API、跨域调用、认证处理 |
...
| `docs/assets/js/lib/html2canvas.min.js` | HTML 转 Canvas 库，用于截图和素材编辑 | HTML 转 Canvas、截图 |
| `docs/assets/js/lib/jszip.min.js` | ZIP 压缩库，用于文件压缩和解压缩 | ZIP 压缩、文件处理 |
...
| `docs/assets/js/lib/marked.min.js` | Markdown 解析库，用于解析帮助文档 | Markdown 解析、帮助文档 |
| `docs/assets/js/lib/pako.min.js` | 压缩库，用于 SVGA 文件压缩和解压缩 | 压缩库、SVGA 处理、第三方库 |
| `docs/assets/js/lib/pngquant.wasm` | PNG 压缩库（WASM），用于 PNG 优化 | PNG 压缩、WASM |
