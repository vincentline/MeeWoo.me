...
| `src/assets/js/components/material-editor-ui.js` | 素材编辑器 UI 组件，提供素材编辑的可视化界面 | 素材编辑器、UI组件、可视化编辑 |
...
| `src/assets/js/service/timer-service.js` | 定时器服务，统一管理所有定时任务，支持延迟/周期/轮询/重试等模式 | TimerService、定时器管理、内存优化 |
| `src/assets/js/service/material-editor/` | 素材编辑器服务模块目录 | 素材编辑、服务模块 |
| `src/assets/js/service/material-editor/text-renderer.js` | 文字渲染服务，支持将文字渲染到Canvas，支持渐变填充、文字阴影、文字描边等特效 | TextRenderer、文字渲染、Canvas、样式处理 |
| `src/assets/js/service/material-editor/image-transformer.js` | 图片变换服务，支持图片加载、缩放、裁剪、填充模式转换（cover/contain） | ImageTransformer、图片变换、cover/contain |
| `src/assets/js/service/material-editor/material-composer.js` | 素材合成服务，将底图和文字层合成为最终图片，支持PNG/JPEG格式导出 | MaterialComposer、素材合成、Konva |
| `src/assets/js/service/material-editor/material-editor-service.js` | 素材编辑器服务主类，组合TextRenderer、ImageTransformer、MaterialComposer三个服务 | MaterialEditorService、编辑器服务、统一入口 |
| `src/assets/js/service/material-editor/index.js` | 素材编辑器服务模块入口，统一导出所有素材编辑相关服务 | 模块入口、服务导出 |
| `src/assets/js/service/material-editor/README.md` | 素材编辑器服务使用文档，包含各服务 API 说明和调用示例 | 服务文档、API 说明、调用示例 |
...
| `src/assets/js/service/frame-capture-service.js` | 帧捕获服务，统一处理动画帧的跳转、等待、捕获逻辑，提供流式回调接口 | FrameCaptureService、帧捕获、序列帧、WebP导出、GIF导出 |
...
| `.trae/logs/UPDATE_LOG.md` | 更新日志，记录版本更新内容 | 更新日志、版本记录、变更历史 |
