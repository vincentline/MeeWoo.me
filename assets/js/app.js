...
function initApp() {
  ...
  var vueInstance = new Vue({
    ...
    methods: {
      ...
      onStandardMp4WidthChange: function () {
        // 保持比例，修改高度
        var ratio = this.standardMp4Config.aspectRatio;
        if (!ratio) return;

        var newWidth = Math.max(1, Math.min(3000, parseInt(this.standardMp4Config.width) || 0));
        var newHeight = Math.floor(newWidth / ratio);

        this.standardMp4Config.width = newWidth;
        this.standardMp4Config.height = newHeight;
      },

      onStandardMp4HeightChange: function () {
        // 保持比例，修改宽度
        var ratio = this.standardMp4Config.aspectRatio;
        if (!ratio) return;

        var newHeight = Math.max(1, Math.min(3000, parseInt(this.standardMp4Config.height) || 0));
      ...
      openGifPanel: function () {
        ...
        // 智能尺寸逻辑：如果有保存的偏好宽度（非0），则保持宽度并重算高度
        // 否则使用素材原始尺寸
        if (this.gifConfig.width > 0 && this.configManager && this.configManager.get('gifConfig')) {
          // 保持当前 width (可能是恢复的偏好)，按比例更新 height
          if (sourceInfo.width && sourceInfo.height) {
            var ratio = sourceInfo.height / sourceInfo.width;
            this.gifConfig.height = Math.floor(this.gifConfig.width * ratio);
          }
        } else {
          // 无偏好，使用原始尺寸
        ...
        // 帧率逻辑：如果有保存的配置，保持不变；否则使用源帧率
        if (this.configManager && this.configManager.get('gifConfig')) {
          // 保持当前 gifConfig.fps
        } else {
      ...
      extractMp4FramesForDualChannel: async function () {
        ...
        for (var i = 0; i < totalFrames; i++) {
          ...
          try {
            ...
            if (_this.mp4ConvertCancelled) break;
          }
      ...
      openLottieToDualChannelPanel: function () {
        ...
        // 初始化配置
        this.lottieDualChannelConfig.width = this.lottie.originalWidth || 300;
        this.lottieDualChannelConfig.height = this.lottie.originalHeight || 300;
      ...
      openMp4ToSvgaPanel: function () {
        ...
        // 读取配置管理器中保存的配置
        if (this.configManager) {
          var savedConfig = this.configManager.get('mp4ToSvgaConfig');
          if (savedConfig) {
            if (savedConfig.quality) this.mp4ToSvgaConfig.quality = savedConfig.quality;
          }
        }
      ...
      startMp4ToSvgaConversion: async function () {
        ...
        // 防止资源冲突
        // if (this.isConvertingMP4 && this.currentModule === 'mp4') {
        //   alert('无法同时进行多个转换任务：\n\n“MP4转双通道”任务正在占用视频资源。\n请等待该任务完成后再试。');
        //   return;
        // }
        ...
        try {
          ...
          // 保存配置到 configManager
          if (this.configManager) {
            this.configManager.set('mp4ToSvgaConfig', {
              quality: this.mp4ToSvgaConfig.quality
            });
          }
      ...
      extractMp4Frames: async function () {
        ...
        for (var i = 0; i < totalFrames; i++) {
          ...
          try {
            ...
            if (_this.mp4ToSvgaCancelled) break;
          }
      ...
      openLottieToSvgaPanel: function () {
        ...
        // 读取配置管理器中保存的配置
        if (this.configManager) {
          var savedConfig = this.configManager.get('lottieToSvgaConfig');
          if (savedConfig) {
            if (savedConfig.quality) this.lottieToSvgaConfig.quality = savedConfig.quality;
          }
        }
      ...
      startLottieToSvgaConversion: async function () {
        ...
        try {
          ...
          if (this.configManager) {
            this.configManager.set('lottieToSvgaConfig', { quality: quality });
          }
      ...
      openFramesToSvgaPanel: function () {
        ...
        if (this.configManager) {
          ...
          if (savedConfig) {
            if (savedConfig.quality) this.framesToSvgaConfig.quality = savedConfig.quality;
          }
        }
      ...
      startSVGAConversion: async function () {
        ...
        // 保存配置到 configManager
        if (this.configManager) {
          this.configManager.set('svga_quality', this.svgaConfig.quality);
        }
      ...
      startMP4Conversion: async function () {
        ...
        try {
          if (this.configManager) {
            this.configManager.set('mp4_quality', this.mp4Config.quality);
    ...
    computed: {
      ...
      isGlobalTaskRunning: function () {
        return this.isExportingGIF ||
          this.isConvertingMP4 ||
          this.isConvertingSVGA ||
          this.isConvertingMp4ToSvga ||
          this.isConvertingLottieToSvga ||
          this.isConvertingFramesToSvga ||
          this.isConvertingStandardMp4 ||
          this.isConvertingFramesToDualChannel ||
          this.isExportingLottie;
      },
